...
class Woo_Wallet_Balance_Details extends WP_List_Table {
	...
	protected function column_username( $item ) {
		...
		'add'  => '<a href="' . esc_url( add_query_arg( array( 'action' => 'get_add_wallet_balance_template', 'security' => wp_create_nonce( 'woo-wallet-add-balance-template' ), 'user_id' => $item['id'], 'TB_iframe' => false, 'width' => 450, 'height' => 450 ), admin_url( 'admin-ajax.php' ) ) ) . '" class="thickbox">' . esc_html__( 'Add Balance', 'woo-wallet' ) . '</a>',
		);
