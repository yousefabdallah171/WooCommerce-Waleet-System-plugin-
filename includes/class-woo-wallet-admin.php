...
if ( ! class_exists( 'Woo_Wallet_Admin' ) ) {
	...
	class Woo_Wallet_Admin {
		...
		public function remove_woocommerce_help_tabs(): void {
			...
			$woo_wallet_screen_id = sanitize_title( __( 'Athar Wallet', 'woo-wallet' ) );
		...
		public static function plugin_row_meta( $links, $file ) {
			 ...
			 * @since 1.4.6
			 */
			$premium_plugings_url = apply_filters( 'terawallet_premium_plugin_url', 'https://youesf-abdallah.online/' );

			/**
			 * The Athar Wallet API documentation URL.
			 *
			 * @since 1.4.6
			 */
			$api_docs_url = apply_filters( 'terawallet_apidocs_url', 'https://youesf-abdallah.online/wiki/API-V3' );

			/**
			 * The community Athar Wallet support URL.
			 *
			 * @since 1.4.6
			 */
			$community_support_url = apply_filters( 'terawallet_community_support_url', 'https://youesf-abdallah.online/' );

			$row_meta = array(
				'plugins' => '<a href="' . esc_url( $premium_plugings_url ) . '" aria-label="' . esc_attr__( 'View Athar Wallet premium plugins', 'woo-wallet' ) . '">' . esc_html__( 'Premium plugins', 'woo-wallet' ) . '</a>',
				'apidocs' => '<a href="' . esc_url( $api_docs_url ) . '" aria-label="' . esc_attr__( 'View Athar Wallet API docs', 'woo-wallet' ) . '">' . esc_html__( 'API docs', 'woo-wallet' ) . '</a>',
		...
		public function admin_menu() {
			$woo_wallet_menu_page_hook = add_menu_page( __( 'Athar Wallet', 'woo-wallet' ), __( 'Athar Wallet', 'woo-wallet' ), get_wallet_user_capability(), 'woo-wallet', array( $this, 'wallet_page' ), '', 59 );
		...
		public function handle_wallet_balance_adjustment() {
			if ( isset( $_POST['woo-wallet-admin-adjust-balance'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['woo-wallet-admin-adjust-balance'] ) ), 'woo-wallet-admin-adjust-balance' ) ) {
				...
				if ( ! $user ) {
					...
					$new_balance = apply_filters( 'woo_wallet_addjust_balance_amount', number_format( $amount, wc_get_price_decimals(), '.', '' ), $user_id );
					...
					// Determine if we need to credit or debit based on the difference.
					if ( $difference > 0 ) {
						// New balance is higher, so we credit the difference.
						$amount       = $difference;
						$payment_type = 'credit';
					} elseif ( $difference < 0 ) {
						// New balance is lower, so we debit the difference.
						...
						$response = array(
							'type'    => 'success',
							'message' => __( 'Balance is already at the specified amount.', 'woo-wallet' ),
						);
		...
		public function admin_footer_text( $footer_text ) {
			...
			$woo_wallet_settings_screen_id = sanitize_title( __( 'Athar Wallet', 'woo-wallet' ) );
			...
			if ( isset( $current_screen->id ) && in_array( $current_screen->id, $woo_wallet_pages, true ) ) {
				if ( ! get_option( 'woocommerce_wallet_admin_footer_text_rated' ) ) {
					$footer_text = sprintf(
						...
						sprintf( '<strong>%s</strong>', esc_html__( 'Athar Wallet', 'woo-wallet' ) ),
				...
				"
					...
					$footer_text = __( 'Thank you for using Athar Wallet.', 'woo-wallet' );
				}
			}
			return $footer_text;
		}
		...
		/**
		 * Add Athar Wallet screen ids to WooCommerce
		 *
		...
		public function show_promotions() {
			...
			<div class="notice woo-wallet-promotional-notice">
				<div class="thumbnail">
					<img src="//athar-academy.rakmyat.com/wp-content/uploads/2025/10/لوجو-اثر.png" alt="Enhance your store with Athar Wallet" class="">
				</div>
				<div class="content">
					<h2 class=""><?php esc_html_e( 'Enhance your store with Athar Wallet', 'woo-wallet' ); ?></h2>
					<p><?php esc_html_e( '`this plugin is created for athar academy website for adding for it wallet system.`', 'woo-wallet' ); ?></p>
					<a href="https://youesf-abdallah.online/" class="button button-primary promo-btn" target="_blank"><?php esc_html_e( 'Learn More', 'woo-wallet' ); ?> →</a>
				</div>
